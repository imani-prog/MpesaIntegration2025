...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    return {
        ...
        resolve(source, importer, {attributes, custom, isEntry, skipSelf} = parseAst_js.BLANK) {
            skipSelf ??= true;
            return graph.moduleLoader.resolveId(source, importer, custom, isEntry, attributes || parseAst_js.EMPTY_OBJECT, skipSelf ? [{
                importer,
                plugin,
                source
            }] : null);
...
const {
    ...
    underline,
    yellow
} = pc.createColors(process$1.env.FORCE_COLOR !== '0' && !process$1.env.NO_COLOR);
    ...
    if (skip) {
        ...
        replaceContext = (pluginContext, plugin) => ({
            ...
            resolve: (source, importer, {attributes, custom, isEntry, skipSelf} = parseAst_js.BLANK) => {
                ...
                return moduleLoaderResolveId(source, importer, custom, isEntry, attributes || parseAst_js.EMPTY_OBJECT, skipSelf ? [...skip, {
                    importer,
                    plugin,
                    source
                }] : skip);
