...
const isAbsolute = function (p) {
    ...
    }, decodeBase64 = typeof atob < "u" ? atob : (str) => Buffer.from(str, "base64").toString("utf-8"), percentRegEx = /%/g,
    backslashRegEx = /\\/g, newlineRegEx = /\n/g, carriageReturnRegEx = /\r/g, tabRegEx = /\t/g, questionRegex = /\?/g,
    hashRegex = /#/g;
...
var comma = 44, chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
    intToChar = new Uint8Array(64), charToInt = new Uint8Array(128);
...
function decode(mappings) {
    let {length} = mappings, reader = new StringReader(mappings), decoded = [], genColumn = 0, sourcesIndex = 0,
        sourceLine = 0, sourceColumn = 0, namesIndex = 0;
    do {
...
var LINE_GTR_ZERO = "`line` must be greater than 0 (lines start at line 1)",
    COL_GTR_EQ_ZERO = "`column` must be greater than or equal to 0 (columns start at column 0)", LEAST_UPPER_BOUND = -1,
...
var HMRContext = class {
    ...
    async fetchUpdate(update) {
        ...
        if (!mod) return;
        let fetchedModule, isSelfUpdate = path === acceptedPath,
...
const createInvokeableTransport = (transport) => {
        ...
        return {
            ...
            async invoke(name, data) {
                let promiseId = nanoid(), wrappedData = {
                    ...
                    resolve: resolve$1,
                    reject
                } = promiseWithResolvers(), timeout = transport.timeout ?? 6e4, timeoutId;
        ...
        let invokeableTransport = createInvokeableTransport(transport), isConnected = !invokeableTransport.connect,
            connectingPromise;
        return {
            ...transport,
    ...
    }, ssrModuleExportsKey = "__vite_ssr_exports__", ssrImportKey = "__vite_ssr_import__",
    ssrDynamicImportKey = "__vite_ssr_dynamic_import__", ssrExportAllKey = "__vite_ssr_exportAll__",
    ssrExportNameKey = "__vite_ssr_exportName__", ssrImportMetaKey = "__vite_ssr_import_meta__", noop = () => {
    }, silentConsole = {
        debug: noop,
        error: noop
    }, hmrLogger = {
...
const sourceMapCache = {}, fileContentsCache = {}, evaluatedModulesCache = /* @__PURE__ */ new Set(),
    retrieveFileHandlers = /* @__PURE__ */ new Set(), retrieveSourceMapHandlers = /* @__PURE__ */ new Set(),
...
function supportRelativeURL(file, url) {
    if (!file) return url;
    let dir = posixDirname(slash(file)), match = /^\w+:\/\/[^/]*/.exec(dir), protocol = match ? match[0] : "",
...
function retrieveSourceMapURL(source) {
    ...
    let re = /\/\/[@#]\s*sourceMappingURL=([^\s'"]+)\s*$|\/\*[@#]\s*sourceMappingURL=[^\s*'"]+\s*\*\/\s*$/gm, lastMatch,
        match;
...
var ModuleRunner = class {
    ...
    async directRequest(url, mod, _callstack) {
        let fetchResult = mod.meta, moduleId = fetchResult.id, callstack = [..._callstack, moduleId],
        ...
        let createImportMeta = this.options.createImportMeta ?? createDefaultImportMeta,
            modulePath = cleanUrl(file || moduleId), href = posixPathToFileHref(modulePath),
            meta = await createImportMeta(modulePath), exports = Object.create(null);
